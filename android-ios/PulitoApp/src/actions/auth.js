...
try{
    ...
    try {
        const body = JSON.stringify({refresh: AsyncStorage.getItem('refresh')})
...
export const login = (number_phone, password) => async dispatch => { 
const config = {
headers:{
    'Content-Type': 'application/json'
}
}
const body = JSON.stringify({number_phone, password});
try {
